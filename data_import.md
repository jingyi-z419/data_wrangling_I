...
``` r
skimr::skim(litters_df)  #not used that much
```
...
``` r
litters_df = read_csv("./data/FAS_litters.csv", skip = 10, col_names = FALSE) #used when the dataset's first row is not column names
```
...
``` r
litters_df = read_csv("./data/FAS_litters.csv", na = c("", "NA", ".", 999)) #see missing values
```
...
``` r
litters_base #by default to print the whole dataset, more difficult to look at
```
...
``` r
litters_readr #more easy to manage, in general use read_csv instead of read.csv
```
